# -*- coding: utf-8 -*-
"""NLP4Health FINAL RAG.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1SV_YB5s4AQ8v-PwX7e7UEKXf23BP7Muc
"""
...
# ==================== SETTINGS ====================
EXTRACT_PATH = "/content/test_data_release/test_data_release"  # extracted test zip
FAISS_DIR = "/content/faiss_indices"
...
# Languages
languages = ['English', 'Assamese', 'Bangla', 'Tamil', 'Kannada',
             'Gujarati', 'Dogri', 'Hindi', 'Marathi', 'Telugu']
...
def is_yes_no_question(question):
    ...
    question = question.strip().lower()
    if any(question.startswith(word + " ") for word in yes_no_starters):
        if not any(word in question for word in ["how","why","explain","tell","describe"]):
            return True
    return False

def retrieve_context(query, lang, top_k=5):
    """Retrieve top-K contexts safely from FAISS for a given language"""
    index_path = os.path.join(FAISS_DIR, f"{lang}_index.faiss")
    map_path = os.path.join(FAISS_DIR, f"{lang}_map.npy")

    if not os.path.exists(index_path) or not os.path.exists(map_path):
        print(f"⚠️ FAISS index or map missing for {lang}")
        return []

    index = faiss.read_index(index_path)
    data_map = np.load(map_path, allow_pickle=True)

    # Load all questions from QnA JSONs
    qna_path = os.path.join(EXTRACT_PATH, lang, "QnA")
    if not os.path.exists(qna_path):
        print(f"⚠️ QnA folder missing for {lang}")
        return []

    json_files = [f for f in os.listdir(qna_path) if f.endswith(".json")]
    all_qas = []
    for jf in json_files:
        with open(os.path.join(qna_path, jf), "r", encoding="utf-8") as f:
            data = json.load(f)
    ...
    for i in I[0]:
        idx = int(data_map[i])
        if idx < len(all_qas):
            context_items.append(all_qas[idx])
    return context_items

def generate_answer(query, lang, context_items, max_tokens=200):
    """Generate answer using simple heuristics or LLM"""
    # --- Yes/No heuristic ---
    if is_yes_no_question(query):
        context_text = " ".join([c["answer"] for c in context_items])
        if any(w in context_text.lower() for w in ["no","not","never","avoid","don't","cannot"]):
            return "No"
        else:
            return "Yes"
    ...
    inputs = tokenizer(prompt, return_tensors="pt", truncation=True, padding=True, max_length=1024).to(device)
    outputs = generator.generate(**inputs, max_new_tokens=max_tokens, num_beams=5, early_stopping=True)
    full_text = tokenizer.decode(outputs[0], skip_special_tokens=True)

    # --- Extract only the answer after the last 'Answer:' ---
...
def save_answer(lang, json_name, questions_with_answers):
    """Save answers to output/<lang>/QnA/<json_name>.json"""
    lang_out_dir = os.path.join(OUTPUT_DIR, lang, "QnA")
    os.makedirs(lang_out_dir, exist_ok=True)

    out_path = os.path.join(lang_out_dir, json_name)
    json.dump({"questions": questions_with_answers}, open(out_path, "w", encoding="utf-8"), ensure_ascii=False, indent=2)
...
print("\n✅ All languages processed and answers saved in output/<lang>/QnA/")
...
languages = ['Dogri', 'Hindi', 'Marathi', 'Telugu']
...
def generate_answer(query, lang, context_items, max_tokens=300):
    """Generate detailed answer using the retrieved context"""
    if len(context_items) == 0:
        return f"माफ करें, {lang} में कोई संदर्भ उपलब्ध नहीं है।"  # Fallback message in the language

    context_text = "\n".join([f"Q: {c['question']} A: {c['answer']}" for c in context_items])
    prompt = (
        f"You are a knowledgeable and helpful medical assistant. Answer the question in {lang} "
        f"based on the context below. Provide a clear, detailed, and easy-to-understand answer.\n\n"
        f"Context:\n{context_text}\n\nQuestion:\n{query}\nAnswer:"
    )
    ...
    # Extract answer after last 'Answer:' if exists
    if "Answer:" in full_text:
        answer = full_text.split("Answer:")[-1].strip()
    else:
        answer = full_text.strip()
...
print("\n✅ Dogri, Hindi, Marathi, and Telugu processed with detailed answers and saved in output/<lang>/QnA/")
